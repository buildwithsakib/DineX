...
if (current_user($pdo)) {
    redirect('/dinex/admin/cashier/dashboard.php');
}

$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_csrf();
    if (!check_rate_limit('cashier_login', 5, 300)) {
        ...
        if (!validate_email($email) || $password === '') {
            ...
            $user = attempt_login($pdo, $email, $password, ROLE_CASHIER);
            if ($user && !empty($user['restaurant_id'])) {
                redirect('/dinex/admin/cashier/dashboard.php');
            } else {
                record_attempt('cashier_login');
                $error = 'Invalid credentials or cashier access denied.';
            }
        }
    }
}
...
<head>
    ...
    <title>Cashier Login - DineX</title>
...
<body class="bg-slate-900 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500">Cashier Login</p>
        </div>
