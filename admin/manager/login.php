...
if (current_user($pdo)) {
    redirect('/dinex/admin/manager/dashboard.php');
}

$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_csrf();
    if (!check_rate_limit('manager_login', 5, 300)) {
        ...
        if (!validate_email($email) || $password === '') {
            ...
            $user = attempt_login($pdo, $email, $password, ROLE_MANAGER);
            if ($user && !empty($user['restaurant_id'])) {
                redirect('/dinex/admin/manager/dashboard.php');
            } else {
                record_attempt('manager_login');
                $error = 'Invalid credentials or manager access denied.';
            }
        }
    }
}
...
<head>
    ...
    <title>Manager Login - DineX</title>
...
<body class="bg-slate-900 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500">Manager Login</p>
        </div>
