...
<head>
    ...
    <title>Instant Lottery | DineX</title>
    ...
    <style>
        ...
        .scratch-card:active { transform: scale(0.98); }
    </style>
</head>
<body class="bg-animated min-h-[100dvh] flex flex-col items-center justify-between py-6 px-4 text-white">
    ...
    <div class="text-center z-10 mt-4">
        ...
        <p class="text-gray-300 text-sm font-medium">Scratch to reveal your prize!</p>
    </div>

    <div class="z-10 my-8 w-full max-w-sm">
        <div id="scratchArea" class="scratch-card rounded-2xl p-8 cursor-pointer select-none text-center mx-auto" onclick="play()">
            <span id="scratchText" class="text-2xl font-bold text-gray-700">🎫 Click to Scratch</span>
        </div>
    </div>

    <div class="z-10 w-full max-w-sm text-center mb-4">
        <button id="actionBtn" onclick="play()" class="spin-btn w-full text-white font-black text-xl py-4 rounded-2xl flex items-center justify-center gap-3 tracking-wide">
            <i class="fa-solid fa-ticket"></i> <span>SCRATCH NOW</span>
        </button>
    ...
    <script>
        ...
        function updateDailyLockUI() {
            if (hasPlayedToday()) {
                ...
                document.getElementById('scratchArea').classList.add('opacity-50','pointer-events-none');
            }
        }
        ...
        async function play() {
            ...
            const btn = document.getElementById('actionBtn');
            btn.disabled = true;
            btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> <span>SCRATCHING...</span>';
            document.getElementById('scratchText').innerText = 'Scratching...';
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (data.success) {
                        ...
                        createConfetti();
                        openWinModal(data);
                        document.getElementById('scratchText').innerText = '🎉 Winner!';
                    } else {
                        playTone(300, 'sawtooth', 0.3);
                        setTimeout(() => playTone(200, 'sawtooth', 0.5), 250);
                        openLoseModal(data.message);
                        document.getElementById('scratchText').innerText = '😅 Try Again';
                    }
                } catch (error) {
                    openLoseModal('Network error. Please try again.');
                    document.getElementById('scratchText').innerText = '🎫 Click to Scratch';
                } finally {
                    isPlaying = false;
                    btn.innerHTML = '<i class="fa-solid fa-ticket"></i> <span>SCRATCH NOW</span>';
                    updateDailyLockUI();
                }
            }, 800);
        }
